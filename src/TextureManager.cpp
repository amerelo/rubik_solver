...
void
TextureManager::generateDefaultTexture(GLuint textureID, GLuint t_width, GLuint t_height)
{
    GLuint      iter_width, iter_height = 0;
    GLubyte     custom_texture[t_width][t_height][4];

    for (iter_width = 0; iter_width < t_width; iter_width++)
    {
        for (iter_height = 0; iter_height < t_height; iter_height++)
        {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, t_width, t_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, custom_texture);
...
void TextureManager::loadAlphaTexture(GLuint textureID, const char *file)
{
    int             width, height, bpp;
    unsigned char*  image = stbi_load(file, &width, &height, &bpp, 1);
...
void TextureManager::loadAtlas(GLuint textureID, char const *filename, int count, int wanted_bpp)
{
	...
	for (int x = 0; x < count; ++x)
	{
		glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0,
						...
						GL_RGB, GL_UNSIGNED_BYTE, image + (x * 256) * 3); // get the pointer on the texture data using very advanced arithmetics. (index * width * RGB)
	}
