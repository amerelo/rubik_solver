...
bool ObjectLoader::parseObjectFile()
{
    ...
    if (v_index_array_num_elem > 0)
        v_index_array = new GLuint[v_index_array_num_elem];
    if (uv_index_array_num_elem > 0)
        uv_index_array = new GLuint[uv_index_array_num_elem];
    if (n_index_array_num_elem > 0)
        n_index_array = new GLuint[n_index_array_num_elem];
    if (vertex_array_num_elem > 0)
        vertex_array = new glm::vec3[vertex_array_num_elem];
    if (uv_array_num_elem > 0)
        uv_array = new glm::vec2[uv_array_num_elem];
    else
    {
        /// In case there is no UV, it sets UV to 0.0f 0.0f.
        uv_array = new glm::vec2[1];
        uv_array->x = 0.0f;
        uv_array->y = 0.0f;
        uv_array_num_elem = 1;
        for (int iter_uv_i_a = 0; iter_uv_i_a < uv_index_array_num_elem; ++iter_uv_i_a)
        {
            uv_index_array[iter_uv_i_a] = 1;
        }
    }
    if (normal_array_num_elem > 0)
        normal_array = new glm::vec3[normal_array_num_elem];
...
void ObjectLoader::fitIndexesToOpenGL()
{
    ...
    for (int iter_v_index = 0; iter_v_index < v_index_array_num_elem; ++iter_v_index)
        v_index_array[iter_v_index] -= 1;
    for (int iter_uv_index = 0; iter_uv_index < uv_index_array_num_elem; ++iter_uv_index)
        uv_index_array[iter_uv_index] -= 1;
    for (int iter_n_index = 0; iter_n_index < n_index_array_num_elem; ++iter_n_index)
        n_index_array[iter_n_index] -= 1;
}

// TODO fix vertex -> normal -> uv once for all
void ObjectLoader::createFATInterleavedData()
{
    ...
    interleaved_fat_data_array = new GLfloat[interleaved_fat_data_num_elem];
    for (int iter_ifda = 0; iter_ifda < interleaved_fat_data_num_elem; iter_ifda += 8)
    {
...
void ObjectLoader::printParsedData()
{
    if (vertex_array != nullptr && uv_array != nullptr && normal_array != nullptr)
    {
        ...
        for (int iter_v_a = 0; iter_v_a < vertex_array_num_elem; iter_v_a++)
        {
        ...
        for (int iter_uv_a = 0; iter_uv_a < uv_array_num_elem; iter_uv_a++)
        {
        ...
        for (int iter_n_a = 0; iter_n_a < normal_array_num_elem; iter_n_a++)
        {
        ...
        for (int iter_v_i_a = 0; iter_v_i_a < v_index_array_num_elem; iter_v_i_a++)
        {
        ...
        for (int iter_n_i_a = 0; iter_n_i_a < n_index_array_num_elem; iter_n_i_a++)
        {
...
void ObjectLoader::freeTemporaryMemory()
{
    delete[] vertex_array;
    delete[] uv_array;
    delete[] normal_array;
    delete[] v_index_array;
    delete[] uv_index_array;
    delete[] n_index_array;
}

void ObjectLoader::freeInterleavedData()
{
    /// delete check for nullptr itself.
    if (data_formatting_status == INTERLEAVED_FAT_DATA)
        delete[] interleaved_fat_data_array;
    if (data_formatting_status == INDEXED_INTERLEAVED_DATA)
    {
        delete[] iid.array;
        delete[] iid.index;
    }
